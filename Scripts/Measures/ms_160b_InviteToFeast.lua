function AIDecision()
	if GetHomeBuilding("","MyHouse") then
		if GetProperty("MyHouse","MusicLevel") and GetProperty("MyHouse","FoodLevel") then
			local Value = GetProperty("MyHouse","MusicLevel") + GetProperty("MyHouse","FoodLevel")
			if GetFavorToDynasty("Destination","") < 30 and Value < 7 then
				return "C"
			else
				return "O"
			end
...
function Run()
	...
	if not GetHomeBuilding("Destination","TheirHome") then
		StopMeasure()
	end
	if not HasProperty("MyHome","InvitationsLeft") then
		StopMeasure()
	end
	if not BuildingGetCity("MyHome","MoiCity") then
		StopMeasure()
	end
	if not BuildingGetCity("TheirHome","TheirCity") then
		StopMeasure()
	end
	if GetID("MoiCity") ~= GetID("TheirCity") then
		MsgQuick("","@L_FEAST_INVITE_FAILURE_ONE")
		StopMeasure()
	end
