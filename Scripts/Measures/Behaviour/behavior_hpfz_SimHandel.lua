function Run()
	if (SimGetNeed("", 7) > 0.7) and (SimGetNeed("", 9) > 0.6) and (Rand(2) == 0) then
		return
	end
	...
	SatisfyNeed("Owner",7,0.14)
	SatisfyNeed("Owner",9,-0.1)
end
...
function KundeAuswahl()
	...
	local itemX, mengeX, slotX, feil, gPreis, summe, bonus, charm, replacev
	local r = 0
	local MinFactor = 0
...
--MsgSayNoWait("","@L  case 1: high ")
--Sleep(2)
			--ÀÎº¥ÀÇ ¹°°ÇÀ» ÆÈ¶§ Ÿà°£ Žõ ºñœÎ°Ô ÆÈ Œö ÀÖŽÙ.
			...
			summe = math.floor(feil + bonus + gPreis)
			replacev = math.floor(gPreis+((gPreis/80) * (feil + bonus)))
			summe = math.max(summe, replacev)
...
-- Sleep(2)
		...
		if AliasExists("CartPointer") then
			...
			-- end
			if ItemGetCategory(lagInhalt[einkauf])~=0 then--and ItemGetCategory(lagInhalt[einkauf])~=6 then
				feil = GetSkillValue("Actor",9) * 1.5
				bonus = SimGetRank("Owner")
				gPreis = ItemGetBasePrice(lagInhalt[einkauf])
				summe = math.floor((feil + bonus) + gPreis)
				replacev = math.floor(gPreis+((gPreis/100) * (feil + bonus)))
				summe = math.max(summe, replacev)
				CreditMoney("Actor",summe,"Offering")
				IncrementXPQuiet("Actor",4)
