...
function Host()
	...
	local Favor = MusicFavor[MusicQuality] + FoodFavor[FoodQuality]
	for i=1,6 do
		if HasProperty("PartyLocation","Guest"..i) then
			local GuestID = GetProperty("PartyLocation","Guest"..i)
			if GetAliasByID(GuestID,"Guest") then
