function Init()
    ...
    was = MsgBox("All","Owner","@P"..wahlButt.."@B[C,@L_REN_FASTMERCHANTSHIP_CANCEL_+0]","@L_REN_FASTMERCHANTSHIP_TITLE_+0","@L_REN_FASTMERCHANTSHIP_KOPF_+0")
	
	if was == "C" then
		StopMeasure()
	end
	
	wieviel = MsgBox("All","Owner","@P@B[20,@L_REN_FASTMERCHANTSHIP_AMOUNT_+0]"..
	...
	whereto = MsgBox("All","Owner","@P@B[X,@L_REN_FASTMERCHANTSHIP_CHOOSEBUTTON_+0]"..
    ...
    if GetRemainingInventorySpace("Ablade",was,INVENTORY_STD) < wieviel then
	    MsgNewsNoWait("All","Owner","","economy",-1,"@L_REN_FASTMERCHANTSHIP_FEHLER_+0",
	...
	for sim=0,SimCount-1 do
		...
		if GetDynasty(SimAlias,"Player") then
			if DynastyIsPlayer("Player") then	
				if kosten > GetMoney(SimAlias) then
	    				MsgNewsNoWait(SimAlias,"Owner","","economy",-1,"@L_REN_FASTMERCHANTSHIP_FEHLER_+0",
	...
	runit = MsgBox("All","Owner","@P@B[A,@L_REN_FASTMERCHANTSHIP_WAHL_+0,]@B[B,@L_REN_FASTMERCHANTSHIP_WAHL_+1,]","","@L_REN_FASTMERCHANTSHIP_RUMPF_+0",wieviel,itName,kosten,GetID("Ablade"),handlerteil,hafenzoll,kontorteil)
	if runit == "B" then
	    StopMeasure()
	end
	...
	for sim=0,SimCount-1 do
		...
		if GetDynasty(SimAlias,"Player") then
			if DynastyIsPlayer("Player") then	
				if kosten > GetMoney(SimAlias) then-- Paranoia-Fix, evt. wartet All zu lange bei Bestellung
	    				MsgNewsNoWait("Owner","Owner","","economy",-1,"@L_REN_FASTMERCHANTSHIP_FEHLER_+0",
	                    		"@L_REN_FASTMERCHANTSHIP_FEHLER_+2")
					StopMeasure()
				end
			end
