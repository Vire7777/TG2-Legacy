function Run()
	...
	if CityGetPenalty("CityAlias", "destination", PENALTY_DEATH, true, "Penalty") then
	    MsgQuick("", "@L_TRIAL_NOT_POSSIBLE_+0")   
		StopMeasure()
	end	
	
	if HasProperty("CityAlias","TrialBooked") then
		MsgQuick("", "@L_TRIAL_NOT_POSSIBLE_BUG_+0")  
		StopMeasure()
	end
	
	if GetState("Destination", STATE_ANIMAL) or GetState("", STATE_ANIMAL) then
		StopMeasure()
	end
		
	-- local Hour = math.mod(GetGametime(), 24)
	-- if Hour > 1 and Hour < 21 then
		-- MsgQuick("", "@L_TRIAL_NOT_POSSIBLE_TIMEBUG_+0") 
		-- StopMeasure()
	-- end
	...
	if not IsGUIDriven() and Rand(100)>25 then 
		...
		if success==0 then
			AddImpact("destination", "IsCharged",1,-1)
			AddImpact("", "HasCharged",1,-1)
			SimChargeCharacter("","destination")
			SetProperty("CityAlias","TrialBooked",1)
		end
		StopMeasure()
	end
