...
function Run()
	...
	if RemoveItems("","Mixture",1)>0 then
		...
		CreateScriptcall("StartDisease",1,"Measures/Artefacts/as_222_UseMixture.lua","StartDisease","Destination",nil)
		
		--let the Destination run away
		GetFleePosition("Destination", "", 500, "Away")
...
function StartDisease()
	--infect
	if GetImpactValue("","staffofaesculap")==0 then
