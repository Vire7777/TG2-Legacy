function Init()
	...
	if setzen > 0 then
		...
		if not HasProperty("","MyMula") then
			SetProperty("","MyMula",setzen)
		else
			local Spent = GetProperty("","MyMula")
			local Spending = Spent + setzen
			SetProperty("","MyMula",Spending)
		end
		if not HasProperty("","NotFinished") then
			SetProperty("","NotFinished",1)
		end
...
function AllEinsatz(pre,zusatz)
  ...
  local pott = GetData("Pott")
	...
	if pre == 2 then
		...
		if not HasProperty("","MyMula") then
			SetProperty("","MyMula",neuSatz)
		else
			local Spent = GetProperty("","MyMula")
			local Spending = Spent + neuSatz
			SetProperty("","MyMula",Spending)
		end
...
function Auszahlung(code)
	...
	if code == 1 then
		...
		RemoveProperty("","NotFinished")  
		RemoveProperty("","MyMula")
...
function CleanUp()
	...
	if HasProperty("","NotFinished") then
		local PayBack = GetProperty("","MyMula")
		if PayBack ~= nil then
			CreditMoney("",PayBack,"IncomeOther")
		end
	end
	RemoveProperty("","NotFinished")
	RemoveProperty("","MyMula")
