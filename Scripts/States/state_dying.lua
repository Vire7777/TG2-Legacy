...
-- -----------------------
function Run()
	AddImpact("","MoveSpeed",0.6,-1)
	feedback_MessageCharacter("", "@L_FAMILY_6_DYING_MSG_NEAR_DEATH_HEAD", "@L_FAMILY_6_DYING_MSG_NEAR_DEATH_BODY", GetID(""))
	...
	if GetState("", STATE_FIGHTING) then
		SetState("", STATE_FIGHTING, false)
	elseif GetState("", STATE_HIJACKED) then
		SetState("", STATE_HIJACKED, false)
	elseif GetState("", STATE_CAPTURED) then
		SetState("", STATE_CAPTURED, false)
	elseif GetState("", STATE_IMPRISONED) then
		SetState("", STATE_IMPRISONED, false)
	elseif GetState("", STATE_WORKING) then
		SetState("", STATE_WORKING, false)
	end
