...
function CreateCityAnimals(city,startup)
	if worldambient_CheckAmbient()==true then
		local count = 1
		if CityGetRandomBuilding(city, GL_BUILDING_CLASS_MARKET, -1, -1, -1, FILTER_IGNORE, "Market") then
			if startup then
...
function CreateAnimal(animal,building,count)	
	if worldambient_CheckAmbient()==true then
		...
		if BuildingGetType(building)==-1 then
			...
			if BuildingGetCity(building, "City") then
				GetLocatorByName(building, "Entry1", "SetPos")
			end
		end

		if CityGetRandomBuilding("City",5,14,-1,-1,FILTER_IGNORE,"Home") then
